    ...
    public static void main(String[] args) {
        ...
        while (true) {
            try (Socket sock = ss.accept()) {
                ...
                synchronized (Upr15Server.odobreni) {
                    Integer i = Upr15Server.odobreni.get(doc.name);
                    if (i == null) {
                        i = 0;
                    }
                    Upr15Server.odobreni.put(doc.name, i + 1);
                ...
                System.out.println("Secretary sent wrong "
                        + "object in odobreni thread");
            }
        }
    }
}
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(this.sock.getOutputStream())) {
            ...
            try (ObjectInputStream in = new ObjectInputStream(this.sock.getInputStream())) {
                ...
                if (doctype.equals("normal")) {
                    synchronized (Upr15Server.normalDocs) {
                        Upr15Server.normalDocs.add(doc);
                    }
                    System.out.println("New normal document saved.");
                } else {
                    synchronized (Upr15Server.specialDocs) {
                        Upr15Server.specialDocs.add(doc);
                    }
    ...
    public void run() {
        try (ObjectOutputStream out = new ObjectOutputStream(this.sock.getOutputStream())) {
            ...
            try (ObjectInputStream in = new ObjectInputStream(this.sock.getInputStream())) {
                ...
                Document doc;
                synchronized (Upr15Server.specialDocs) {
                    if (!Upr15Server.specialDocs.isEmpty()) {
                        out.writeUTF("special");
                        out.flush();
                        doc = Upr15Server.specialDocs.getFirst();
                        out.writeObject(doc);
                        out.flush();
                        Upr15Server.specialDocs.removeFirst();
                ...
                synchronized (Upr15Server.normalDocs) {
                    if (!Upr15Server.normalDocs.isEmpty()) {
                        out.writeUTF("normal");
                        out.flush();
                        doc = Upr15Server.normalDocs.getFirst();
                        out.writeObject(doc);
                        out.flush();
                        Upr15Server.normalDocs.removeFirst();
